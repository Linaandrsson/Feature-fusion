...
batch_size = 128  # Larger batch for better BN statistics
epochs = 200
lr = 5e-4  # Lower learning rate

# Early stopping
patience = 15  # More patience for slower learning
min_delta = 1e-4
...
class ECGCNN(nn.Module):
    def __init__(self, num_classes: int, seq_len: int, num_channels: int):
        ...
        self.bn_embed = nn.BatchNorm1d(128)  # Normalize embeddings

        # Classification head
        self.drop_cls = nn.Dropout(0.6)  # Very high dropout
    ...
    def extract_features(self, x):
        ...
        x = self.flatten(x)
        z = self.fc_embed(x)
        z = self.bn_embed(z)  # Apply batch normalization
        return z
...
for variant_dir in variant_dirs:
    ...
    # Subject-based splits — consistent with training and fusion ablation script
    train_idx_var = np.where(~np.isin(subjects_variant, TEST_SUBJECTS + VAL_SUBJECTS))[0]
    val_idx_var   = np.where(np.isin(subjects_variant, VAL_SUBJECTS))[0]
    test_idx_var  = np.where(np.isin(subjects_variant, TEST_SUBJECTS))[0]
    print(f"   Subject-based splits for {variant_name}: train={len(train_idx_var)} val={len(val_idx_var)} test={len(test_idx_var)}")
