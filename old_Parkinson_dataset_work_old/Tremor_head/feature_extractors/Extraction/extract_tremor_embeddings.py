...
Extracts 128-dim embeddings from trained Acc_arm, Gyro_arm, and Mag_arm CNNs.

Binary tremor classification (0=no tremor, 1=tremor present).
...
# ═══════════════════════════════════════════════════════════════
# SETUP PATHS BEFORE IMPORTING TRAINING MODULES
# ═══════════════════════════════════════════════════════════════
# This must happen before we import from training scripts
...
parkinson_dataset_work_dir = tremor_head_dir.parent  # /.../.../Parkinson_dataset_work/

TRAINING_SCRIPTS_DIR = "s2_w2_tremor_fs50"
training_scripts_path = tremor_head_dir / "feature_extractors" / "Training" / TRAINING_SCRIPTS_DIR
sys.path.insert(0, str(training_scripts_path))

# Use Parkinson_dataset_work as base for data paths
WORKSPACE_ROOT = parkinson_dataset_work_dir
...
MODEL_DIR = WORKSPACE_ROOT / "Tremor_head" / "feature_extractors" / "Models" / "s2_w2_tremor" / f"{SAMPLING_FREQ}_mixed3"
...
# Sensors and model paths (three sensors for tremor detection)
SENSORS = {
    "Acc_arm": MODEL_DIR / "feature_extractor_Acc_arm.pth",
    "Gyro_arm": MODEL_DIR / "feature_extractor_Gyro_arm.pth",
    "Mag_arm": MODEL_DIR / "feature_extractor_Mag_arm.pth"
}

# Import config from training scripts (path was set up above)
from config import MANUAL_SUBJECT_SPLITS
...
# Split configuration (must match training - from MANUAL_SUBJECT_SPLITS)
TEST_SUBJECTS = MANUAL_SUBJECT_SPLITS["test"]  # [7, 9, 18, 19, 27]
VAL_SUBJECTS = MANUAL_SUBJECT_SPLITS["val"]    # [6, 8, 13, 15, 25]
TRAIN_SUBJECTS = MANUAL_SUBJECT_SPLITS["train"] # [2, 3, 4, 5, 10, 11, 12, 14, 16, 17, 20, 21, 22, 23, 24, 26, 28, 29]
...
def split_data_by_subject(
    ...
    subject_splits: Dict[str, List[int]]
) -> Tuple[Dict, Dict, Dict]:
    """Split data by subject IDs using MANUAL_SUBJECT_SPLITS."""
    subjects = data["y_subject"]
    
    train_mask = np.isin(subjects, subject_splits["train"])
    val_mask = np.isin(subjects, subject_splits["val"])
    test_mask = np.isin(subjects, subject_splits["test"])
...
def extract_sensor_embeddings(sensor_name: str, model_path: Path, variant_path: Path, output_dir: Path):
    ...
    Args:
        sensor_name: Sensor name (Acc_arm, Gyro_arm, or Mag_arm)
    ...
    # Dynamically import the correct model class (delayed import to avoid conflicts)
    ...
    # Split by subject (using MANUAL_SUBJECT_SPLITS)
    train_data, val_data, test_data = split_data_by_subject(data, MANUAL_SUBJECT_SPLITS)
    
    # DEBUG: Print subject splits
    actual_train_subjects = sorted(np.unique(train_data['y_subject']).tolist())
    actual_val_subjects = sorted(np.unique(val_data['y_subject']).tolist())
    actual_test_subjects = sorted(np.unique(test_data['y_subject']).tolist())
    print(f"    DEBUG - Train subjects: {actual_train_subjects}")
    print(f"    DEBUG - Val subjects: {actual_val_subjects}")
    print(f"    DEBUG - Test subjects: {actual_test_subjects}")
...
def main():
    ...
    print(f"Tremor classification: BINARY (0=no tremor, 1=tremor present)")
