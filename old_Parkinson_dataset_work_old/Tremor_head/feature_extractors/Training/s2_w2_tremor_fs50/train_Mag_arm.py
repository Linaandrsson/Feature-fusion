"""
Tremor Feature Extractor Training: Mag_arm CNN
================================================

Trains a CNN feature extractor for Mag_arm sensor on tremor dataset.
The model learns to classify binary tremor presence (0=no tremor, 1=tremor) and can extract 128-dim embeddings.

IMPORTANT: Tremor labels are remapped from severity levels (0-5) to binary:
  - 0 → 0 (no tremor)
  - 1-5 → 1 (tremor present, any severity)

Architecture (same as data fusion model):
  - Conv1d(3, 64, kernel_size=5) + BatchNorm + ReLU + MaxPool
  - Conv1d(64, 128, kernel_size=5) + BatchNorm + ReLU + MaxPool
  - Flatten + Activity embedding concatenation
  - fc_embed: Linear(combined_dim, 128)  # 128-dim feature embedding
  - fc_cls: Linear(128, 2)  # 2 binary tremor classes
...
def merge_dataset_variants(sensor_name: str, dataset_variants: List[str], base_path: Path) -> Dict[str, np.ndarray]:
    ...
    # Remap to binary for display
    tremor_binary = (merged['y_tremor_score'] > 0).astype(np.int64)
    print(f"  Binary tremor distribution: {np.bincount(tremor_binary)}")
...
class TremorDataset(Dataset):
    """Dataset for tremor binary classification (0=no tremor, 1=tremor)."""
    ...
    def __init__(self, data: Dict[str, np.ndarray], norm_stats: Dict[str, np.ndarray]):
        ...
        # IMPORTANT: Remap tremor scores to binary: 0 stays 0, 1-5 becomes 1
        self.y_score = (data["y_tremor_score"] > 0).astype(np.int64)
...
class MagArmFeatureExtractor(nn.Module):
    """
    CNN feature extractor for Mag_arm sensor.
...
def main():
    ...
    print("=" * 80)
    print("TRAINING MAG_ARM FEATURE EXTRACTOR FOR TREMOR CLASSIFICATION")
    print("=" * 80)
    ...
    # Split by subject (using explicit subject lists from MANUAL_SUBJECT_SPLITS)
    ...
    print("=" * 80)
    model = MagArmFeatureExtractor().to(device)
    print(model)
