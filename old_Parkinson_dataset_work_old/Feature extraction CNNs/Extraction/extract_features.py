...
class IMUCNN(nn.Module):
    ...
    def __init__(
        ...
        self.features = nn.Sequential(
            nn.Conv1d(num_channels, conv1_channels, kernel_size=5, padding=2),
        ...
        self.flattened_dim = (seq_len // 4) * conv2_channels
...
def load_data(sensor_name: str, num_channels: int):
    ...
    y_activity = data[:, -7].astype(int) - 1  # Activity label 1..16 -> 0..15
    ...
    # Load variant-local splits (matching this variant's row count)
    train_idx = np.loadtxt(get_split_path("train"), dtype=int)
    val_idx = np.loadtxt(get_split_path("val"), dtype=int)
    test_idx = np.loadtxt(get_split_path("test"), dtype=int)
...
def build_model_from_checkpoint(ckpt: dict, seq_len: int, num_channels: int, fallback_num_classes: int, device: torch.device) -> IMUCNN:
    """Instantiate IMUCNN with channel sizes inferred from checkpoint weights."""
    state = ckpt["model_state_dict"]
    conv1_channels = int(state["features.0.weight"].shape[0])
    conv2_channels = int(state["features.5.weight"].shape[0])
    num_classes = int(ckpt.get("num_classes", fallback_num_classes))
