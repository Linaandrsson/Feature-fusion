...
Usage:
  python v2_run_ablation_multirun_two_gpus.py                           # 10 seeds, GPU 1
  python v2_run_ablation_multirun_two_gpus.py --n_runs 10
  python v2_run_ablation_multirun_two_gpus.py --seeds 42,43,44          # explicit seeds
  python v2_run_ablation_multirun_two_gpus.py --gpus 0,1 --n_runs 10
"""
...
SCRIPT = Path(__file__).parent / "v2_run_ablation_parallel.py"
...
def main():
    ...
    parser.add_argument(
        "--corrupt_test_variant", type=str, default=None,
        help="Corrupt variant used for test evaluation, passed through to ablation script"
    )
    ...
    for run_idx, seed in enumerate(seeds, 1):
        ...
        if args.corrupt_test_variant is not None:
            cmd += ["--corrupt_test_variant", args.corrupt_test_variant]
    ...
    if failed_seeds:
        ...
        print(f"Results in: {SCRIPT.parent}/corruption_logs/")
