...
SCRIPT = Path(__file__).parent / "v2_fusion_concat_ablation_study_tremor.py"
...
def main():
    ...
    corrupt_str = corrupt_test_variant if corrupt_test_variant else "(default from script)"
    ...
    log_dir = SCRIPT.parent / "corruption_logs" / "process_logs"
    ...
    for k in k_values:
        ...
        if seed is not None:
            cmd += ["--seed", str(seed)]
        if corrupt_test_variant is not None:
            cmd += ["--corrupt_test_variant", corrupt_test_variant]
    ...
    if failed:
        ...
        print(f"Reports + CSVs saved in: {SCRIPT.parent}/corruption_logs/")
