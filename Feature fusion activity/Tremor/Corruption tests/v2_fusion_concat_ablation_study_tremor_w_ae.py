...
ABLATION_REPORT_TAG = "clean_train_awgn_a020"
...
corrupt_test_variant = "s4_w4_fs50_tremor_clean_awgn_a020"   # fallback for sensors not in corrupt_test_mix
...
# -------------------------------
# Autoencoder fusion configuration
# -------------------------------
# When USE_AUTOENCODER_FUSION=True, the concatenated sensor embeddings are passed
# through an MLP autoencoder bottleneck before classification. The classifier
# operates on the compact latent vector z, not the raw concatenation.
# This tests whether a learned compact fused representation improves robustness.
# Set to False to use the original concat-only behaviour (backward compatible).
USE_AUTOENCODER_FUSION = False

AE_LATENT_DIM   = 128   # Bottleneck size: 256 / 128 / 64 / 32
AE_HIDDEN_DIM   = 256   # Intermediate size in encoder and decoder MLPs
AE_RECON_LAMBDA = 0.01  # Weight of reconstruction loss: total = CE + lambda * MSE
AE_DROPOUT      = 0.2   # Dropout inside the encoder (0.0 = disabled)
...
class AutoencoderFusionModel(nn.Module):
    """
    Fusion model with an MLP autoencoder bottleneck between feature concatenation
    and classification.

    The encoder compresses the concatenated sensor embeddings into a compact latent
    vector z. The classifier operates on z instead of the raw concatenation, learning
    a more compact and potentially more robust fused representation. The decoder
    reconstructs the original concatenated embeddings from z during training only.

    Architecture:
        concat_embeddings [concat_dim]
          → encoder: Linear(concat_dim, hidden_dim) → ReLU → Dropout → Linear(hidden_dim, latent_dim) → ReLU
          → latent z [latent_dim]
          → classifier head (PaperHead) → logits
          → decoder: Linear(latent_dim, hidden_dim) → ReLU → Linear(hidden_dim, concat_dim) → reconstruction

    Training loss:
        total_loss = CrossEntropyLoss(logits, labels)
                   + ae_recon_lambda * MSELoss(reconstruction, concat_embeddings.detach())

    The model is sensor-subset-agnostic: concat_dim = embed_dim * num_sensors is
    computed dynamically per ablation combination, so it works for any subset size.
    """
    ...
    def __init__(
        ...
        # Encoder: concatenated embeddings → compact latent z
        encoder_layers: list = [
            nn.Linear(concat_dim, hidden_dim),
            nn.ReLU(),
        ]
        if ae_dropout > 0:
            encoder_layers.append(nn.Dropout(ae_dropout))
        encoder_layers += [
        ...
        # Decoder: latent z → reconstructed concatenated embeddings
        ...
        # Classifier head operates on latent z (not raw concat)
        self.head = PaperHead(
            in_dim=latent_dim,
    ...
    def forward(self, Z_list):
        # Concatenate per-sensor embeddings
        x = torch.cat(Z_list, dim=1)       # [B, concat_dim]
        # Encode to compact latent representation
        z = self.encoder(x)                # [B, latent_dim]
        # Reconstruct original concatenation from latent space (used for recon loss)
        x_hat = self.decoder(z)            # [B, concat_dim]
        # Classify from latent representation (not raw concat)
...
def train_sensor_combination(
    ...
    if USE_AUTOENCODER_FUSION:
        concat_dim = embed_dim * num_sensors
        ...
        print(f"  Mode:              AUTOENCODER FUSION")
        print(f"  Concat dim:        {concat_dim}")
        print(f"  Latent dim:        {AE_LATENT_DIM}")
        print(f"  Compression ratio: {concat_dim / AE_LATENT_DIM:.1f}x")
        print(f"  AE hidden dim:     {AE_HIDDEN_DIM}")
        print(f"  Recon lambda:      {AE_RECON_LAMBDA}")
    ...
    for epoch in range(epochs):
        ...
        for batch in train_loader:
            ...
            if USE_AUTOENCODER_FUSION:
                out    = model(Z)
                logits = out["logits"]
                recon_loss = F.mse_loss(out["reconstruction"], out["concat_embeddings"].detach())
                loss = criterion(logits, y_batch) + AE_RECON_LAMBDA * recon_loss
            else:
                logits, _ = model(Z)
    ...
    results = {
        ...
        "num_sensors": len(sensors),
        "fusion_mode": "autoencoder" if USE_AUTOENCODER_FUSION else ("gated" if USE_GATING else "concat"),
        "ae_latent_dim":   AE_LATENT_DIM   if USE_AUTOENCODER_FUSION else None,
        "ae_recon_lambda": AE_RECON_LAMBDA if USE_AUTOENCODER_FUSION else None,
