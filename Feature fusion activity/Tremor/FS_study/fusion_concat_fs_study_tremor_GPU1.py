"""
fusion_concat_fs_study_tremor_GPU1.py

Tremor FS Combination Study — GPU 1 — Scenario: Clean→Tremor

Tests all sampling-frequency combinations for a fixed sensor list.
Trains on clean embeddings, tests on all tremor variants (clean + mild_mod + mod_severe).

GPU isolation: GPU_ID = 1  →  device = cuda:1
Log directory: tremor_logs_gpu1/
Log file:      tremor_fusion_fs_combo_study_clean_tremor.jsonl

Run alongside GPU0 script without any crossover:
...
# -----------------------------------------------------------------------
# Scenario configuration — Clean→Tremor
# -----------------------------------------------------------------------
...
# -----------------------------------------------------------------------
TRAIN_TREMOR_TYPES = ["tremor_clean"]                                              # train on clean only
TEST_TREMOR_TYPES  = ["tremor_clean", "tremor_mild_mod", "tremor_mod_severe"]      # test on all tremor variants
...
# -------------------------------
# Logging — GPU1-specific directory and filename
# -------------------------------
LOG_DIR = SCRIPT_DIR / "tremor_logs_gpu1"
LOG_DIR.mkdir(parents=True, exist_ok=True)
LOG_FILE = LOG_DIR / "tremor_fusion_fs_combo_study_clean_tremor.jsonl"
...
def main():
    ...
    print("=" * 80)
    print(f"TREMOR FS COMBINATION STUDY  [GPU {GPU_ID}]  —  Clean→Tremor")
    print("=" * 80)
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Tremor FS combination study — GPU1 — Clean→Tremor")
