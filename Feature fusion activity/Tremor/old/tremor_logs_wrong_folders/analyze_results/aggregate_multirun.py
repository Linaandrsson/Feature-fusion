...
def discover_tags() -> list[str]:
    """Return all tag names found in LOGS_DIR (ablation_json_files_<tag> dirs)."""
    prefix = "ablation_json_files_"
    tags = sorted(
        d.name[len(prefix):]
...
def main():
    available_tags = discover_tags()

    parser = argparse.ArgumentParser(description="Aggregate multi-seed ablation results")
    parser.add_argument(
        "--tag", type=str, action="append", dest="tags",
        metavar="TAG",
        help=(
            "Which tag to aggregate. Can be repeated for multiple tags. "
            f"Available: {available_tags}"
        ),
    )
    parser.add_argument(
        "--all", action="store_true",
        help=f"Aggregate all available tags: {available_tags}",
    )
    ...
    if args.all or args.both:
        tags_to_run = available_tags if args.all else ["mixed", "clean"]
    elif args.tags:
        tags_to_run = args.tags
    else:
        # default: run all discovered tags
        tags_to_run = available_tags
