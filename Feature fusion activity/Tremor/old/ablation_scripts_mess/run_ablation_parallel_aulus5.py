"""
run_ablation_parallel_aulus5.py

Launches all 8 ablation_k sizes (k=1..8) as parallel subprocesses,
distributing across 2 GPUs (round-robin).
Uses fusion_concat_ablation_study_tremor_aulus5.py as the ablation script.
...
Usage:
  python run_ablation_parallel_aulus5.py [--gpus 0,1]
"""
...
SCRIPT = Path(__file__).parent / "fusion_concat_ablation_study_tremor_aulus5.py"
...
def main():
    ...
    parser.add_argument(
        ...
        help="Override ABLATION_REPORT_TAG in the ablation script (e.g. --tag mixed)"
    )
    parser.add_argument(
        "--variants", type=str, default=None,
        help="Override tremor_variants as comma-separated list"
    )
    ...
    print("=" * 60)
    print(f"Tremor Ablation Parallel Launcher (aulus5)  [{timestamp}]")
    print("=" * 60)
    ...
    log_dir = Path(__file__).parent / "tremor_logs" / "process_logs"
    ...
    if failed:
        ...
        print(f"Reports + CSVs saved in: {Path(__file__).parent}/tremor_logs/")
