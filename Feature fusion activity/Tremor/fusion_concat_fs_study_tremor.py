"""
fusion_concat_fs_study_tremor.py

Tremor FS Combination Study (updated structure)

Tests all sampling-frequency combinations for a fixed sensor list,
using the same tremor dataset structure and subject-based splitting as:
- fusion_concat_ablation_study_tremor.py

For each FS combination:
  1) Load per-sensor embeddings from tremor variants (clean/mild_mod/mod_severe)
  2) Re-split by subject IDs (TRAIN/VAL/TEST)
  3) Train fusion model (gated or concat)
  4) Log metrics to JSONL

Log file:
  Feature fusion activity/Tremor/tremor_logs/tremor_fusion_fs_combo_study.jsonl
"""
...
# -------------------------------
FIXED_SENSORS = ["Acc_ankle", "Mag_arm", "Mag_ankle"]  # Fixed sensor set for all experiments (3-sensor combos)  
FS_CANDIDATES = [10, 20, 30, 40, 50]
MAX_COMBOS = None
SKIP_EXISTING = True

# Tremor variants used as augmentation (same structure as ablation script)
...
LOG_FILE = LOG_DIR / "tremor_fusion_fs_combo_study.jsonl"
...
def get_variant_names_for_fs(fs: int) -> List[str]:
    return [f"s2_w2_fs{fs}_{tremor_type}" for tremor_type in TREMOR_TYPES]
...
def load_combined_tremor_embeddings(sensor_name: str, fs: int) -> Dict[str, np.ndarray]:
    """
    Load + combine embeddings across tremor variants for one sensor and one fs,
    then re-split by subject IDs to avoid leakage.
    """
    cache_key = (sensor_name, fs, EMBEDDINGS_FOLDER_NAME)
    ...
    variant_names = get_variant_names_for_fs(fs)
    ...
    val_subjects = np.unique(subj_all[val_mask]).tolist()
    test_subjects = np.unique(subj_all[test_mask]).tolist()
    ...
    if sorted(val_subjects) != expected_val:
        raise ValueError(f"Val subjects mismatch for {sensor_name} fs{fs}: {val_subjects} != {expected_val}")
    if sorted(test_subjects) != expected_test:
        raise ValueError(f"Test subjects mismatch for {sensor_name} fs{fs}: {test_subjects} != {expected_test}")
...
def run_one_experiment(
    ...
    for sensor in sensors:
        variants = get_variant_names_for_fs(fs_map[sensor])
        print(f"  {sensor} (fs{fs_map[sensor]}): {variants}")

    # 1) Load embeddings for each sensor at its assigned fs
    embeddings_dict = {}
    for sensor in sensors:
        fs = fs_map[sensor]
        embeddings_dict[sensor] = load_combined_tremor_embeddings(sensor, fs)

    # 2) Verify alignment
    verify_label_alignment(embeddings_dict, sensors)

    embed_dim = embeddings_dict[sensors[0]]["Z_train"].shape[1]
    num_classes = len(np.unique(embeddings_dict[sensors[0]]["y_train"]))

    # 3) Build datasets
    def prepare_tensors(split: str):
        Z_key = f"Z_{split}"
        y_key = f"y_{split}"
    ...
    Z_train, y_train = prepare_tensors("train")
    Z_val, y_val = prepare_tensors("val")
    Z_test, y_test = prepare_tensors("test")
...
def main():
    ...
    print("=" * 80)
    print("TREMOR FS COMBINATION STUDY")
    print("=" * 80)
    ...
    print(f"  Tremor types merged: {TREMOR_TYPES}")
    ...
    if collected_results:
        ...
        print("\nTop this run:")
        print(f"  Best Test F1: {best['test_f1']:.4f} | FS map: {best['fs_map']}")
        print(f"  Worst Test F1: {worst['test_f1']:.4f} | FS map: {worst['fs_map']}")
