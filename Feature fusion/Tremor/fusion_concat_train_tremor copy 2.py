...
def load_combined_tremor_embeddings(sensor_name: str) -> Dict[str, np.ndarray]:
    ...
    Each variant folder contains:
      Tremor_ExtractedFeatures/{sensor}_embeddings.npz with:
        - train_embeddings, train_subjects (subjects 1,3,4,6,8,9)
        - val_embeddings, val_subjects (subjects 2,7)
        - test_embeddings, test_subjects (subjects 5,10)
        - train_labels, val_labels, test_labels (TREMOR SEVERITY SCORES 0-4)
        - train_activities, val_activities, test_activities (not used for tremor classification)
    
    Returns dict with combined (concatenated) data:
      Z_train, y_train, Z_val, y_val, Z_test, y_test
      where y_* are TREMOR SEVERITY SCORES (0-4), not activities
    """
    Z_train_all = []
    Z_val_all = []
    Z_test_all = []
    ...
    for variant_name in tremor_variants:
        ...
        # Use TREMOR LABELS (severity scores 0-4) for classification
        if len(y_train_all) == 0:
            # First variant - save tremor severity labels
            y_train_all = data["train_labels"].astype(np.int64)
            y_val_all = data["val_labels"].astype(np.int64)
            y_test_all = data["test_labels"].astype(np.int64)
        else:
            # Verify tremor labels match across variants (should be same for same subjects)
            if not np.array_equal(data["train_labels"], y_train_all):
                raise ValueError(f"Train tremor label mismatch in {variant_name} for {sensor_name}")
            if not np.array_equal(data["val_labels"], y_val_all):
                raise ValueError(f"Val tremor label mismatch in {variant_name} for {sensor_name}")
            if not np.array_equal(data["test_labels"], y_test_all):
                raise ValueError(f"Test tremor label mismatch in {variant_name} for {sensor_name}")
    ...
    # Repeat tremor severity labels for each variant
