"""
Calculate RMS for Tremor Dataset Windows
=========================================

Calculate Root Mean Square (RMS) values for all sensors in the tremor dataset.
Supports clean, mild_mod, and mod_severe variants.
...
# Sensor information
SENSORS = {
    "Acc_chest": {"axes": 3, "unit": "m/s²", "label": "Accelerometer Chest"},
    "ECG": {"axes": 2, "unit": "mV", "label": "ECG Chest"},
    "Acc_ankle": {"axes": 3, "unit": "m/s²", "label": "Accelerometer Ankle"},
    "Gyro_ankle": {"axes": 3, "unit": "deg/s", "label": "Gyroscope Ankle"},
    "Mag_ankle": {"axes": 3, "unit": "μT", "label": "Magnetometer Ankle"},
    "Acc_arm": {"axes": 3, "unit": "m/s²", "label": "Accelerometer Arm"},
    "Gyro_arm": {"axes": 3, "unit": "deg/s", "label": "Gyroscope Arm"},
    "Mag_arm": {"axes": 3, "unit": "μT", "label": "Magnetometer Arm"}
}
...
VARIANTS = {
    'clean': 's2_w2_fs50_tremor_clean',
    'mild': 's2_w2_fs50_tremor_mild_mod',
    'severe': 's2_w2_fs50_tremor_mod_severe'
}

# Sensors without tremor (chest sensors - always have score=0)
TREMOR_FREE_SENSORS = {'Acc_chest', 'ECG'}

SCRIPT_DIR = Path(__file__).parent.resolve()
DATA_BASE = SCRIPT_DIR.parent.parent / "data" / "Tremor_datagenerator_files"
OUTPUT_DIR = SCRIPT_DIR / "rms_results_tremor"
...
def get_activity_window(variant_key: str, activity_idx: int, window_idx: int = 0, score: float = None):
    """
    Get a specific window for a given activity from all sensors.
    ...
    # Step 1: Use Acc_arm to find the global index (it has tremor metadata)
    try:
        ref_data = load_sensor_data(variant_name, 'Acc_arm')
    ...
    # Step 2: Load all sensors using the global index
    result = {}
    ...
    # Add metadata (use Acc_arm since Acc_chest has no tremor metadata)
    # Chest sensors are TREMOR_FREE_SENSORS, so we use arm/ankle for tremor labels
    result['metadata'] = {
        'variant': variant_key,
        ...
        'subject_id': result['Acc_arm']['subject_id'],
        'tremor_freq': result['Acc_arm']['tremor_freq'],
        'tremor_acc_rms': result['Acc_arm']['tremor_acc_rms'],
        'tremor_gyro_rms': result['Acc_arm']['tremor_gyro_rms'],
        'tremor_score': result['Acc_arm']['tremor_score']
    }
...
def calculate_sensor_rms(sensor_signal, num_axes):
    ...
    Args:
        ...
        num_axes: Number of axes (2 or 3)
        
    Returns:
        Dictionary with per-axis RMS and magnitude RMS (for 3-axis sensors)
    """
    rms_values = {}
    ...
    # Calculate total magnitude RMS for 3-axis sensors
    # RMS_total = sqrt(RMS_x^2 + RMS_y^2 + RMS_z^2)
    if num_axes == 3:
...
def calculate_all_rms(window_data):
    """
    Calculate RMS for all sensors in a window.
...
def print_rms_results(results):
    ...
    print("\n" + "="*80)
    print("RMS ANALYSIS RESULTS - TREMOR DATASET")
    print("="*80)
    ...
    for sensor_name in SENSORS.keys():
        if sensor_name in results['sensors']:
            ...
            if num_axes == 3:
                ...
                print(f"  Lead I RMS:      {rms['axis_0']:>10.4f} {unit}")
                print(f"  Lead II RMS:     {rms['axis_1']:>10.4f} {unit}")
            print()
...
def export_rms_to_txt(results, output_dir=None, custom_filename=None):
    ...
    Args:
        ...
        output_dir: Output directory (default: rms_results_tremor)
    ...
    if custom_filename is None:
        filename = f"rms_{meta['variant']}_act{meta['activity_idx']}_win{meta['window_idx']}.txt"
    else:
    ...
    lines = []
    lines.append("=" * 80)
    lines.append("RMS ANALYSIS RESULTS - TREMOR DATASET")
    lines.append("=" * 80)
    ...
    for sensor_name in SENSORS.keys():
        if sensor_name in results['sensors']:
            ...
            if num_axes == 3:
                ...
                lines.append(f"  Lead I RMS:      {rms['axis_0']:>10.4f} {unit}")
                lines.append(f"  Lead II RMS:     {rms['axis_1']:>10.4f} {unit}")
            lines.append("")
...
def main():
    ...
    print("="*60)
    print("RMS ANALYSIS - TREMOR DATASET")
    print("="*60)
    ...
    # Calculate RMS
    print("\nCalculating RMS for all sensors...")
