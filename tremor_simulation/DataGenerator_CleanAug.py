"""
Data Generator for Clean Augmented Datasets
============================================

This script generates augmented variants of the CLEAN (tremor-free) dataset.
All generated data is "clean healthy" — no pathological tremor is introduced.

Two augmentation strategies are applied (one variant each):
  1. AWGN (Additive White Gaussian Noise):
       - Adds realistic channel-wise sensor noise relative to signal RMS.
       - Simulates real-world sensor noise variability.
       - Applied to ALL sensors (including ECG).
       - Controlled by SCENARIO_RMS_RATIO (default 0.15 = 15% of signal RMS).

  2. Rotation-based augmentation:
       - Applies a small random 3D rotation to 3-axis sensors (Acc, Gyro, Mag).
       - Simulates realistic sensor orientation variation during attachment.
       - Preserves original motion magnitude and structure.
       - For 2-axis sensors (ECG): falls back to AWGN.
       - Controlled by SCENARIO_MAX_ROTATION_DEG (default 15°).

IMPORTANT:
  - GENERATE_TREMOR is always False: all tremor labels are 0.
  - Signal content is "clean healthy" throughout.
  - Identical folder structure, NPZ/TXT format, and label columns as the tremor variants.

Generated variants:
  - s4_w4_fs50_tremor_clean_awgn  (AWGN augmentation)
  - s4_w4_fs50_tremor_clean_rot   (Rotation augmentation)

Intended use:
  Combine [clean, clean_awgn, clean_rot] as training data to match the 3× data
  volume of the mixed tremor condition, isolating dataset-size from tremor-variation
  effects when comparing ablation results.

Based on DataGenerator_Tremor.py — all tremor-related infrastructure is preserved
so tremor-branch outputs and file formats remain identical.
"""
...
RUN_SANITY_CHECKS = False        # No tremor to check in clean-aug variants
NUM_DIAGNOSTIC_WINDOWS = 10      # Number of windows to inspect for diagnostics

# ============================================================
# CONFIG - Tremor Generation
# ============================================================
# Always False for this script: clean healthy data only, all tremor labels = 0
GENERATE_TREMOR = False

# ============================================================
# CONFIG - Scenario Augmentation
# ============================================================
# SCENARIO_AUGMENT: controls which clean-data augmentation strategy to apply
#   None       : no scenario augmentation (same as original clean variant)
#   "awgn"     : Additive White Gaussian Noise on all sensors
#   "rotation" : Random 3D rotation on 3-axis sensors; AWGN fallback for ECG
#
# Applied BEFORE resampling (on raw sensor values), ensuring augmentation is
# scale-independent. All tremor labels remain 0 — signal stays "clean healthy".
SCENARIO_AUGMENT = None          # Set per-variant in __main__; not changed directly
SCENARIO_RMS_RATIO = 0.15        # AWGN: noise RMS as fraction of signal RMS per channel
SCENARIO_MAX_ROTATION_DEG = 15.0 # Rotation: maximum rotation angle in degrees
SCENARIO_SEED = 100              # Seed for scenario augmentation RNG (separate from SEED)
...
def generate_tremor_dataset(variant_name: str, augment_mode: str = None):
    ...
    scenario_rng = np.random.default_rng(SCENARIO_SEED)  # Separate RNG for scenario augmentation
    ...
    for sensor_name, cols in SENSORS.items():
        ...
        for w_idx, (subj_idx, label, win_start, win_end) in enumerate(window_specs):
            ...
            # ---- Scenario augmentation (clean-aug variants) ----
            # Applied to ALL sensors, ALL windows regardless of tremor score.
            # Tremor labels remain 0; this is purely clean healthy signal augmentation.
            if SCENARIO_AUGMENT == "awgn":
                win_raw = apply_awgn_raw(win_raw, scenario_rng, rms_ratio=SCENARIO_RMS_RATIO)
            elif SCENARIO_AUGMENT == "rotation":
                if len(cols) >= 3:
                    # 3-axis sensors: apply random rotation (preserves motion structure)
                    win_raw = apply_rotation_augmentation(
                        win_raw, scenario_rng, max_angle_deg=SCENARIO_MAX_ROTATION_DEG
                    )
                else:
                    # 2-axis sensors (ECG): rotation undefined, fall back to AWGN
                    win_raw = apply_awgn_raw(win_raw, scenario_rng, rms_ratio=SCENARIO_RMS_RATIO)
            # ---- End scenario augmentation ----
    ...
    scenario_rng_tremor = np.random.default_rng(SCENARIO_SEED + 1)  # Separate RNG for scenario aug (tremor branch)
    ...
    for sensor_name, cols in TREMOR_BRANCH_SENSORS.items():
        ...
        for w_idx, (subj_idx, label, win_start, win_end) in enumerate(window_specs):
            ...
            # ---- Scenario augmentation (clean-aug variants, tremor branch) ----
            if SCENARIO_AUGMENT == "awgn":
                win_raw = apply_awgn_raw(win_raw, scenario_rng_tremor, rms_ratio=SCENARIO_RMS_RATIO)
            elif SCENARIO_AUGMENT == "rotation":
                if len(cols) >= 3:
                    win_raw = apply_rotation_augmentation(
                        win_raw, scenario_rng_tremor, max_angle_deg=SCENARIO_MAX_ROTATION_DEG
                    )
                else:
                    win_raw = apply_awgn_raw(win_raw, scenario_rng_tremor, rms_ratio=SCENARIO_RMS_RATIO)
            # ---- End scenario augmentation ----
    ...
    info_lines.extend([
        "",
        "=" * 80,
        "Scenario Augmentation (clean-aug specific):",
        "=" * 80,
        f"  SCENARIO_AUGMENT       : {SCENARIO_AUGMENT if SCENARIO_AUGMENT is not None else 'None (no scenario augmentation)'}",
        f"  SCENARIO_RMS_RATIO     : {SCENARIO_RMS_RATIO}",
        f"  SCENARIO_MAX_ROTATION  : {SCENARIO_MAX_ROTATION_DEG} degrees",
        f"  SCENARIO_SEED          : {SCENARIO_SEED}",
        "",
        "  Applied BEFORE resampling on raw sensor values.",
        "  Signal content remains 'clean healthy' throughout (no pathological tremor).",
        "",
        "=" * 80,
        "Dataset Format:",
        "=" * 80,
...
if __name__ == "__main__":
    # Generate two clean-augmented variants of the clean dataset.
    # Both use GENERATE_TREMOR = False (all tremor labels = 0).
    # Format: s{STRIDE}_w{WINDOW}_fs{FS}_tremor_clean_{augment_type}
    
    fs_str = f"fs{int(FS)}"
    base_name = f"s{int(STRIDE_SEC)}_w{int(WINDOW_SEC)}_{fs_str}_tremor_clean"
    
    # Define the two augmentation scenarios to generate
    scenarios = [
        ("awgn",     "AWGN (Additive White Gaussian Noise)"),
        ("rotation", "Rotation-based augmentation"),
    ]
    
    print("\n" + "="*80)
    print(f"GENERATING {len(scenarios)} CLEAN-AUGMENTED VARIANTS")
    print("="*80)
    print(f"Base name: {base_name}_[type]")
    print(f"Variants: {', '.join(s for s, _ in scenarios)}")
    print(f"GENERATE_TREMOR: {GENERATE_TREMOR} (all tremor labels = 0)")
    print("="*80 + "\n")
    
    for idx, (aug_type, aug_desc) in enumerate(scenarios, 1):
        print("\n" + "#"*80)
        print(f"#  VARIANT {idx}/{len(scenarios)}: {aug_desc}")
        print("#"*80 + "\n")
        
        # Set the global scenario augmentation type for this run
        SCENARIO_AUGMENT = aug_type
        
        variant_name = f"{base_name}_{aug_type}"
        generate_tremor_dataset(variant_name, augment_mode="clean")
        
        print("\n" + "#"*80)
        print(f"#  \u2713 COMPLETED VARIANT {idx}/{len(scenarios)}: {variant_name}")
        print("#"*80 + "\n")
    
    print("\n" + "="*80)
    print(f"\u2705 ALL {len(scenarios)} CLEAN-AUGMENTED VARIANTS GENERATED SUCCESSFULLY")
    print("="*80)
    print("Generated variants:")
    for aug_type, _ in scenarios:
        print(f"  \u2713 {base_name}_{aug_type}")
    print("="*80 + "\n")
