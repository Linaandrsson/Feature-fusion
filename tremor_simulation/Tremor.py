...
def simulate_and_add_tremor_imu(
    ...
    2) resample to fs
    3) build s_acc and s_gyro from x1/x2
    4) optional intermittent envelope
    5) project acc and gyro along a shared 3D unit direction v
    6) magnetometer tremor via rotation driven by the projected gyroscope tremor signal
       (NOT additive noise — rotation-based physical model)
    7) return corrupted signals + signals/metadata for logging/debug

    Args:
        freq_hz: Tremor frequency in Hz (e.g., 4.5 for 4.5 Hz Parkinson tremor)
                 Range typically 3.5-7.0 Hz for Parkinson's disease
        mag_rms: Kept for API compatibility but no longer used for additive noise.
                 Magnetometer tremor is derived entirely from the gyroscope tremor
                 via cumulative rotation (apply_tremor_rotation_to_magnetometer).
    """
    ...
    # 3) choose which state drives acc and gyro
    def pick_state(which: str):
        return x1_s if which == "x1" else x2_s

    s_acc  = pick_state(use_acc_state)   # s_acc  = x1 (position state)
    s_gyro = pick_state(use_gyro_state)  # s_gyro = x2 (velocity state, 90° phase shift)

    # 4) scale to desired RMS
    s_acc  = scale_to_rms(s_acc,  acc_rms)
    s_gyro = scale_to_rms(s_gyro, gyro_rms)

    # 5) optional intermittent envelope (same on/off for all modalities)
    ...
    # 6) shared direction vector for acc and gyro additive projection
    v = unit_random_direction(rng)

    # --- Accelerometer: additive projected tremor ---
    X_acc_t = add_projected_tremor(X_acc, s_acc, v)

    # --- Gyroscope: additive projected tremor ---
    # Build the (N, 3) gyroscope tremor signal before adding to X_gyro.
    # This 3D signal is also passed to the magnetometer rotation model.
    gyro_tremor_3d = s_gyro[:, None] * v[None, :]   # (N, 3), in deg/s
    X_gyro_t = X_gyro + gyro_tremor_3d

    # --- Magnetometer: rotation-based tremor (NOT additive noise) ---
    # Physical model: magnetometer readings change because tremor rotates the sensor.
    # The gyroscope tremor signal drives cumulative incremental rotations applied to
    # the magnetometer vector at each timestep (Rodrigues formula, see
    # apply_tremor_rotation_to_magnetometer for details).
    # mag_rms is intentionally NOT used here.
    X_mag_t = apply_tremor_rotation_to_magnetometer(X_mag, gyro_tremor_3d, fs)
    ...
    meta = {
        ...
        "s_acc": s_acc,
        "s_gyro": s_gyro,
        "gyro_tremor_3d": gyro_tremor_3d,  # (N,3) signal used for mag rotation
        "mu": mu,
        "sigma": sigma,
        "dt": dt,
        "freq_hz": freq_hz,
...
def precompute_tremor_cache_with_parkinson_model(
    ...
    if sampling_method == "subject":
        ...
        print(f"       activity_beta = get_activity_beta_for_score(score, activity)")
        print(f"       RMS_acc = RMS_acc_base * activity_beta")
    ...
    if sampling_method == "subject":
        ...
        print(f"  - Activity beta: score-dependent (BETA_ACTIVITY_BY_SCORE)")
    ...
    for (w_idx, body_part), (subj_idx, label, win_start, win_end) in body_parts_to_process.items():
        ...
        if sampling_method == "subject":
            ...
            acc_base_rms_val = None  # base RMS before activity scaling (arm/chest only)
            ...
            if body_part == 'ankle':
                ...
                activity_beta = float(arm_meta.get('activity_beta', 1.0))
                ...
                acc_base_rms_val = float(acc_target)  # ankle has no separate activity scaling
            else:
                # For arm and chest, sample score and base RMS first,
                # then apply score-dependent activity modulation
                score, acc_base, freq_hz = pk_config.sample_tremor_params(
                    augment_mode=augment_mode,
                    rng=interval_rng
                )

                # Apply score-dependent activity modulation (draw score first, then scale)
                activity_beta = pk_config.get_activity_beta_for_score(score, label)
                acc_target = max(0.0, float(acc_base) * activity_beta)
                acc_base_rms_val = float(acc_base)
        ...
        if sampling_method == "subject":
            ...
            meta['acc_base_rms'] = acc_base_rms_val if acc_base_rms_val is not None else float(acc_target)
            meta['activity_label'] = int(label)
