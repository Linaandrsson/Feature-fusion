...
# Config
# -------------------------------
sensor_name = "Gyro_UR"  # Sensor file name without extension (Parkinson LL = Lower Left)
num_channels = 3
...
def _save_splits(train_i, val_i, test_i):
    np.savetxt(split_save_dir / "train_idx.txt", train_i, fmt='%d')
    np.savetxt(split_save_dir / "val_idx.txt", val_i, fmt='%d')
    np.savetxt(split_save_dir / "test_idx.txt", test_i, fmt='%d')

def _is_valid_index_array(arr, n_samples):
    arr = np.atleast_1d(arr)
    return arr.size > 0 and np.all(arr >= 0) and np.all(arr < n_samples)

if not split_file.exists():
    _save_splits(train_idx, val_idx, test_idx)
    print(f"Saved split indices to: {split_save_dir}")
else:
    # Load existing splits to ensure consistency across sensors.
    # If stale/corrupted for current dataset size, regenerate safely.
    train_loaded = np.atleast_1d(np.loadtxt(split_save_dir / "train_idx.txt", dtype=int))
    val_loaded = np.atleast_1d(np.loadtxt(split_save_dir / "val_idx.txt", dtype=int))
    test_loaded = np.atleast_1d(np.loadtxt(split_save_dir / "test_idx.txt", dtype=int))

    valid = (
        _is_valid_index_array(train_loaded, N)
        and _is_valid_index_array(val_loaded, N)
        and _is_valid_index_array(test_loaded, N)
    )

    if not valid:
        print(f"Existing split indices are stale for N={N}; regenerating: {split_save_dir}")
        _save_splits(train_idx, val_idx, test_idx)
    else:
        train_idx = train_loaded
        val_idx = val_loaded
        test_idx = test_loaded
