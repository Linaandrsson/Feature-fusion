...
RMS Formulas:
    ...
    where k_g is determined by accelerometer RMS severity:
      - 0.05-0.10 m/s²: k_g = 11 (mild)
      - 0.25-0.5 m/s²:  k_g = 22 (mild-moderate)
      - 1.10-2.0 m/s²:  k_g = 14 (moderate-severe)
      - 4.65-5.10 m/s²: k_g = 13 (severe)
...
TREMOR_FREE_SENSORS = ['Mag_head', 'Acc_head', 'Gyro_head']  # Head sensors are less affected by limb tremor
...
# magnetometer vector.
#
# All 4 body location magnetometers follow this physical model:
# - Mag_UR rotation driven by Gyro_UR tremor (Upper Right)
# - Mag_LR rotation driven by Gyro_LR tremor (Lower Right)
# - Mag_UL rotation driven by Gyro_UL tremor (Upper Left)
# - Mag_LL rotation driven by Gyro_LL tremor (Lower Left)

ROTATION_BASED_MAG_SENSORS = ['Mag_UR', 'Mag_LR', 'Mag_UL', 'Mag_LL']
...
#   "subject": Subject-based (old method) NO MORE
...
def choose_kg_from_rms_acc_old(rms_acc: float) -> float:
    ...
    Intervals match Table kg_final:
      Score 1: 0.05–0.10  -> k_g = 12
      Score 2: 0.10–0.5   -> k_g = 22
      Score 3: 0.5–2.0    -> k_g = 14
      Score 4: 2.0–6.0    -> k_g = 13
